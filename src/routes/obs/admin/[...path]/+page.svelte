<script lang="ts">
	...
	const ALL_PERMISSIONS = [
		'users.manage','roles.manage','catalog.manage','sections.manage','calendar.manage','audit.read',
		'grades.get','grades.put','attendance.get','attendance.put',
		'enrollments.post','enrollments.approve',
		'messages.get','messages.post','messages.delete',
		'document_requests.get','document_requests.post','document_requests.manage',
		'announcements.get','announcements.post',
		'exams.get','exams.post',
	...
	let termForm   = { name: '', academic_year: '2025-2026', season: 'spring', starts_at: '', ends_at: '' };
	...
	let sectionForm = {
		course_id: '', term_id: '', instructor_id: '',
		classroom_id: '', section_no: 1,
		day_of_week: 'Pazartesi', start_time: '09:00', end_time: '10:50', capacity: 40,
	};
	...
	async function loadPage(path: string) {
		...
		const loaders: Record<string, TokenFn> = {
			users:        (t) => getDouAdminUsers(t, { role: userRoleFilter || undefined, search: userSearch || undefined }).then(r => { users = r.users; }),
			roles:        (t) => getDouAdminRoles(t).then(r => { roles = r.roles; if (!selectedRoleId && roles.length) { selectedRoleId = roles[0].id; rolePermissions = [...(roles[0].permissions ?? [])]; } }),
			departments:  (t) => getDouDepartments(t).then(r => { departments = r; }),
			terms:        (t) => getDouTerms(t).then(r => { terms = r; }),
		courses:      (t) => getDouCourses(t).then(r => { courses = r; }),
		classrooms:   (t) => getDouClassrooms(t).then(r => { classrooms = r; }),
		sections:     async (t) => {
	...
	async function toggleUserActive(u: AdminUser) {
		const token = localStorage.token ?? null;
		const updated = await patchDouAdminUser(token, u.id, { is_active: !u.is_active }).catch(() => u);
	...
	async function createSection() {
		if (!sectionForm.course_id || !sectionForm.term_id || !sectionForm.instructor_id) {
			sectionErr = 'Ders, dönem ve akademisyen seçimi zorunludur.'; return;
		}
		sectionSaving = true; sectionErr = null;
		const token = localStorage.token ?? null;
		...
		try {
			...
			sectionSaved = true;
			sectionForm = { course_id: '', term_id: '', instructor_id: '', classroom_id: '', section_no: 1, day_of_week: 'Pazartesi', start_time: '09:00', end_time: '10:50', capacity: 40 };
	...
	<div class="space-y-4">
		...
		{:else if apiKey === 'users'}
			<div class="flex flex-wrap items-center gap-3">
				<input bind:value={userSearch} on:input={() => loadPage(activePath)} placeholder="Ad veya e-posta ara…"
				...
				<select bind:value={userRoleFilter} on:change={() => loadPage(activePath)}
				...
				<button on:click={() => showUserModal = true} type="button"
				...
				<div class="overflow-x-auto">
					<table class="w-full text-sm">
						...
						<tbody>
							{#each users as u}
								<tr class="border-t border-black/5 dark:border-white/10 hover:bg-slate-50/50 transition-colors">
									...
									<td class="px-4 py-3 text-center">
										<select value={u.role} on:change={(e) => changeUserRole(u, (e.target as HTMLSelectElement).value)}
									...
									<td class="px-4 py-3 text-center">
										<span class="rounded-full px-2 py-0.5 text-xs font-medium {u.is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'}">
											{u.is_active ? 'Aktif' : 'Pasif'}
										</span>
									</td>
									<td class="px-4 py-3 text-center text-xs text-slate-400">{u.created_at}</td>
									<td class="px-4 py-3 text-center">
										<button on:click={() => toggleUserActive(u)} type="button"
							...
							{:else}
								<tr><td colspan="6" class="px-4 py-8 text-center text-sm text-slate-400">Kullanıcı bulunamadı.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
			...
			{#if showUserModal}
				<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
					<div class="w-full max-w-md rounded-2xl border border-black/10 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-slate-900">
						<div class="mb-5 flex items-center justify-between">
							<div class="font-bold">Yeni Kullanıcı</div>
							<button on:click={() => showUserModal = false} type="button" class="text-slate-400 hover:text-slate-700">✕</button>
						</div>
						<div class="space-y-3">
							{#each [['Ad Soyad','full_name','text'],['E-posta','email','email'],['Parola','password','password']] as [lbl,field,type]}
								<label class="block">
									...
									<input bind:value={newUser[field as keyof typeof newUser]} {type}
							...
							<label class="block">
								...
								<select bind:value={newUser.role} class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-slate-800">
									<option value="Öğrenci">Öğrenci</option>
									<option value="Akademisyen">Akademisyen</option>
									<option value="Admin">Admin</option>
								</select>
							...
							<div class="flex gap-2 pt-2">
								<button on:click={createUser} disabled={userCreating} type="button"
								...
								<button on:click={() => showUserModal = false} type="button"
		...
		{:else if apiKey === 'roles'}
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
				...
				<div class="rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="border-b border-black/5 px-5 py-3 text-xs font-bold text-slate-400 dark:border-white/10">ROLLER</div>
					{#each roles as r}
						<button on:click={() => selectRole(r.id)} type="button"
							class="flex w-full items-center justify-between border-t border-black/5 px-5 py-3 text-left text-sm transition-colors dark:border-white/10
								{selectedRoleId === r.id ? 'bg-sky-50 dark:bg-sky-900/10' : 'hover:bg-slate-50 dark:hover:bg-white/5'}">
							<div>
								<div class="font-semibold">{r.name}</div>
								<div class="text-xs text-slate-400">{r.description}</div>
							</div>
							<span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600 dark:bg-white/10 dark:text-slate-300">{r.permissions.length}</span>
						</button>
					{/each}
				</div>

				<!-- Sağ: Permission checkbox -->
				<div class="col-span-2 rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="mb-3 flex items-center justify-between">
						<div class="font-semibold">{roles.find(r => r.id === selectedRoleId)?.name ?? 'Rol Seç'} — İzinler</div>
						{#if rolesSaved}<span class="text-xs text-emerald-600 dark:text-emerald-400">Kaydedildi ✓</span>{/if}
					</div>
					<div class="grid grid-cols-2 gap-2">
						{#each ALL_PERMISSIONS as perm}
							<label class="flex cursor-pointer items-center gap-2 rounded-lg border border-black/5 px-3 py-2 text-xs hover:bg-slate-50 dark:border-white/10 dark:hover:bg-white/5">
								<input type="checkbox" checked={rolePermissions.includes(perm)}
					...
					<button on:click={saveRolePermissions} disabled={rolesSaving} type="button"
			...
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
				<div class="col-span-2 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="grid grid-cols-3 bg-slate-50 px-5 py-3 text-xs font-bold text-slate-500 dark:bg-white/5 dark:text-slate-400">
						<div>Kod</div><div class="col-span-2">Ad</div>
					</div>
				...
				<div class="rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					{#if formSaved['dept']}<div class="mb-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">Eklendi.</div>{/if}
					<div class="space-y-3">
						<label class="block"><div class="mb-1 text-xs font-semibold text-slate-500">Kod</div>
							<input bind:value={deptForm.code} class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" /></label>
						<label class="block"><div class="mb-1 text-xs font-semibold text-slate-500">Ad</div>
							<input bind:value={deptForm.name} class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" /></label>
						<button on:click={() => genericCreate('dept')} type="button"
							class="w-full rounded-lg bg-sky-500 py-2 text-sm font-semibold text-white hover:bg-sky-400 transition-colors">Ekle</button>
					</div>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
				<div class="col-span-2 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="grid grid-cols-4 bg-slate-50 px-5 py-3 text-xs font-bold text-slate-500 dark:bg-white/5 dark:text-slate-400">
						<div class="col-span-2">Dönem</div><div>Başlangıç</div><div>Bitiş</div>
					</div>
					{#each terms as t}
						<div class="grid grid-cols-4 border-t border-black/5 px-5 py-3 text-sm dark:border-white/10">
							<div class="col-span-2 font-medium">
								{t.name}
								{#if t.is_active}<span class="ml-2 rounded-full bg-sky-100 px-1.5 py-0.5 text-[10px] font-bold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">Aktif</span>{/if}
							</div>
				...
				<div class="rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					{#if formSaved['term']}<div class="mb-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">Eklendi.</div>{/if}
					<div class="space-y-3">
						{#each [['Ad','name','text'],['Akademik Yıl','academic_year','text'],['Başlangıç','starts_at','date'],['Bitiş','ends_at','date']] as [lbl,field,type]}
							<label class="block">
								<div class="mb-1 text-xs font-semibold text-slate-500">{lbl}</div>
								<input bind:value={termForm[field as keyof typeof termForm]} {type}
						...
						{/each}
						<button on:click={() => genericCreate('term')} type="button"
							class="w-full rounded-lg bg-sky-500 py-2 text-sm font-semibold text-white hover:bg-sky-400 transition-colors">Ekle</button>
					</div>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
				<div class="col-span-2 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="overflow-x-auto">
						<table class="w-full text-sm">
							<thead class="bg-slate-50 text-xs font-bold text-slate-500 dark:bg-white/5 dark:text-slate-400">
								<tr><th class="px-4 py-3 text-left">Kod</th><th class="px-4 py-3 text-left">Ad</th><th class="px-4 py-3 text-center">K</th><th class="px-4 py-3 text-center">AKTS</th></tr>
							</thead>
							<tbody>
								{#each courses as c}
									<tr class="border-t border-black/5 dark:border-white/10">
										<td class="px-4 py-3 font-mono text-xs font-semibold text-slate-500">{c.code}</td>
										<td class="px-4 py-3 font-medium">{c.name}</td>
										<td class="px-4 py-3 text-center">{c.credits}</td>
										<td class="px-4 py-3 text-center">{c.akts}</td>
									</tr>
								{:else}<tr><td colspan="4" class="px-4 py-8 text-center text-sm text-slate-400">Ders yok.</td></tr>{/each}
							</tbody>
						</table>
					</div>
				</div>
				<div class="rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					{#if formSaved['course']}<div class="mb-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">Eklendi.</div>{/if}
					<div class="space-y-3">
						{#each [['Kod','code','text'],['Ad','name','text']] as [lbl,field,type]}
							<label class="block">
								<div class="mb-1 text-xs font-semibold text-slate-500">{lbl}</div>
								<input bind:value={courseForm[field as keyof typeof courseForm]} {type}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" />
							</label>
						{/each}
						<div class="grid grid-cols-2 gap-2">
							<label class="block"><div class="mb-1 text-xs font-semibold text-slate-500">Kredi</div>
								<input type="number" bind:value={courseForm.credits} class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" /></label>
							<label class="block"><div class="mb-1 text-xs font-semibold text-slate-500">AKTS</div>
								<input type="number" bind:value={courseForm.akts} class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" /></label>
						</div>
						<button on:click={() => genericCreate('course')} type="button"
							class="w-full rounded-lg bg-sky-500 py-2 text-sm font-semibold text-white hover:bg-sky-400 transition-colors">Ekle</button>
					</div>
				</div>
			</div>
		...
		{:else if apiKey === 'classrooms'}
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
				<div class="col-span-2 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="grid grid-cols-4 bg-slate-50 px-5 py-3 text-xs font-bold text-slate-500 dark:bg-white/5 dark:text-slate-400">
						<div>Kod</div><div class="col-span-2">Ad</div><div>Kapasite</div>
					</div>
				...
				<div class="rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					{#if formSaved['class']}<div class="mb-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">Eklendi.</div>{/if}
					<div class="space-y-3">
						{#each [['Kod','code','text'],['Ad','name','text'],['Bina','building','text']] as [lbl,field,type]}
							<label class="block">
								<div class="mb-1 text-xs font-semibold text-slate-500">{lbl}</div>
								<input bind:value={classForm[field as keyof typeof classForm]} {type}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" />
							</label>
						{/each}
						<label class="block"><div class="mb-1 text-xs font-semibold text-slate-500">Kapasite</div>
							<input type="number" bind:value={classForm.capacity} class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none dark:border-white/10 dark:bg-white/5" /></label>
						<button on:click={() => genericCreate('class')} type="button"
							class="w-full rounded-lg bg-sky-500 py-2 text-sm font-semibold text-white hover:bg-sky-400 transition-colors">Ekle</button>
					</div>
				</div>
			</div>
	...
	{:else if apiKey === 'sections'}
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-5">
			...
			<div class="lg:col-span-2">
				<div class="rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					{#if sectionSaved}
						<div class="mb-4 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
							✓ Şube başarıyla oluşturuldu.
						</div>
					{/if}
					{#if sectionErr}
						<div class="mb-4 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600 dark:bg-red-950/40 dark:text-red-400">
							{sectionErr}
						</div>
					{/if}
					...
					<div class="space-y-3">
						<!-- Ders -->
						<label class="block">
							<span class="mb-1 block text-xs font-semibold text-slate-500">Ders <span class="text-red-400">*</span></span>
							<select bind:value={sectionForm.course_id}
						...
						<!-- Dönem -->
						<label class="block">
							<span class="mb-1 block text-xs font-semibold text-slate-500">Dönem <span class="text-red-400">*</span></span>
							<select bind:value={sectionForm.term_id}
								class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5">
						...
						<!-- Akademisyen -->
						<label class="block">
							<span class="mb-1 block text-xs font-semibold text-slate-500">Akademisyen <span class="text-red-400">*</span></span>
							<select bind:value={sectionForm.instructor_id}
								class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5">
						...
						<div class="grid grid-cols-2 gap-3">
							<label class="block">
								...
								<select bind:value={sectionForm.classroom_id}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5">
							...
							<label class="block">
								...
								<input type="number" min="1" max="10" bind:value={sectionForm.section_no}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5" />
							</label>
						</div>
						...
						<div class="grid grid-cols-3 gap-3">
							<label class="block">
								...
								<select bind:value={sectionForm.day_of_week}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5">
									{#each DAYS as d}
							...
							<label class="block">
								...
								<select bind:value={sectionForm.start_time}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5">
									{#each HOURS as h}
							...
							<label class="block">
								...
								<select bind:value={sectionForm.end_time}
									class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5">
									{#each HOURS as h}
										<option value={h}>{h}</option>
									{/each}
						...
						<label class="block">
							...
							<input type="number" min="1" max="200" bind:value={sectionForm.capacity}
								class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5" />
						</label>

						<button on:click={createSection} disabled={sectionSaving} type="button"
			...
			<div class="lg:col-span-3">
				<div class="rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="flex items-center justify-between border-b border-black/5 px-5 py-3 dark:border-white/10">
						...
						<span class="rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-bold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">
					...
					{:else}
						<div class="overflow-x-auto">
							<table class="w-full text-sm">
								<thead>
									<tr class="border-b border-black/5 bg-slate-50/80 dark:border-white/10 dark:bg-white/5">
										...
										<th class="px-4 py-2 text-left text-xs font-bold text-slate-400">Akademisyen</th>
								...
								<tbody>
									{#each adminSections as sec}
										...
										<tr class="border-t border-black/5 hover:bg-slate-50/50 dark:border-white/10 dark:hover:bg-white/5">
											<td class="px-4 py-2.5 font-mono text-xs font-semibold text-slate-500">{s.course_code ?? '—'}</td>
											<td class="px-4 py-2.5">{s.course_name ?? s.course_id ?? '—'}</td>
											<td class="px-4 py-2.5 text-xs text-slate-600 dark:text-slate-300">{s.instructor_label ?? '—'}</td>
											<td class="px-4 py-2.5 text-xs text-slate-500">{s.day_of_week ?? '—'} {s.start_time ?? ''}</td>
											<td class="px-4 py-2.5 text-xs text-slate-500">{s.classroom_code ?? s.classroom_id ?? '—'}</td>
				...
				<!-- Bilgi kartı -->
				<div class="mt-4 rounded-xl border border-amber-200/60 bg-amber-50/60 px-4 py-3 text-xs text-amber-700 dark:border-amber-900/30 dark:bg-amber-950/20 dark:text-amber-400">
					<strong>Not:</strong> Oluşturulan şubeler ilgili akademisyenin "Şubelerim" listesinde otomatik olarak görünür.
					Öğrenciler ders kaydı aşamasında bu şubelere kayıt yaptırabilir.
				</div>
			</div>
		</div>
		...
		{:else if apiKey === 'reg-rules'}
			<div class="mx-auto max-w-lg rounded-xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
				...
				<div class="space-y-4">
					{#each [
						['AKTS Limiti (varsayılan)', 'akts_limit_default', 'number'],
						['AKTS Limiti (yüksek GNO)', 'akts_limit_high', 'number'],
						['AKTS Limiti (hazırlık)',    'akts_limit_prep', 'number'],
						['Yüksek AKTS için min. GNO', 'min_gpa_for_high_akts', 'number'],
						['Ekle/Bırak süresi (gün)',  'add_drop_deadline_days', 'number'],
					] as [lbl,field,type]}
						<label class="block">
							<div class="mb-1 text-xs font-semibold text-slate-500">{lbl}</div>
							<input {type} bind:value={regRules[field as keyof typeof regRules]} step="0.01"
								class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5" />
						</label>
					{/each}
						...
						<tbody>
							{#each (docRequests as {id:string;document_type:string;document_subtype:string;requesting_institution:string;created_at:string;status:string}[]) as req}
								<tr class="border-t border-black/5 dark:border-white/10">
									...
									<td class="px-4 py-3 text-center">
										<span class="rounded-full px-2.5 py-0.5 text-xs font-medium {req.status === 'tamamlandı' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300'}">
											{req.status}
										</span>
									</td>
									<td class="px-4 py-3 text-right">
										{#if req.status !== 'tamamlandı'}
											<button on:click={() => completeDoc(req.id)} type="button"
							...
							{:else}
								<tr><td colspan="6" class="px-4 py-8 text-center text-sm text-slate-400">Belge talebi yok.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
		...
		{:else if apiKey === 'announce'}
			<div class="mx-auto max-w-lg rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
				...
				<div class="space-y-3">
					...
					<label class="block"><div class="mb-1 text-xs font-semibold text-slate-500">İçerik</div>
						<textarea bind:value={annForm.content} rows="5"
							class="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-400/40 dark:border-white/10 dark:bg-white/5"></textarea></label>
						...
						<tbody>
							{#each (auditLogs as {id:string;action:string;entity_type:string;user:string;created_at:string}[]) as log}
								<tr class="border-t border-black/5 dark:border-white/10 hover:bg-slate-50/50 transition-colors">
									...
									<td class="px-4 py-3">
										<span class="rounded-full bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-600 dark:bg-white/10 dark:text-slate-300">{log.action}</span>
									</td>
									<td class="px-4 py-3 font-mono text-xs text-slate-500">{log.entity_type}</td>
									<td class="px-4 py-3 text-xs font-medium">{log.user}</td>
									<td class="px-4 py-3 text-xs text-slate-400">{log.created_at?.slice(0,16).replace('T',' ')}</td>
								</tr>
							{:else}
								<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-slate-400">Log bulunamadı.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
