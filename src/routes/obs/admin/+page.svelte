...
>
	...
	{#if statsErr}
		<div
			class="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-300"
		>
			{statsErr}
		</div>
	{/if}
	...
	<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
		{#each [{ label: 'Toplam Kullanıcı', value: stats?.users_total ?? (loading ? '…' : '—'), color: 'sky' }, { label: 'Aktif Kullanıcı', value: stats?.users_active ?? (loading ? '…' : '—'), color: 'emerald' }, { label: 'Açık Şube', value: stats?.sections ?? (loading ? '…' : '—'), color: 'violet' }, { label: 'Ders (Katalog)', value: stats?.courses ?? (loading ? '…' : '—'), color: 'amber' }] as card}
			<div
			...
			>
				...
				<div class="mt-1.5 text-2xl font-black text-slate-800 dark:text-slate-100">
					{card.value}
				</div>
			</div>
		{/each}
