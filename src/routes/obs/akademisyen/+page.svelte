<script lang="ts">
	...
	const quickLinks = [
		{ href: '/obs/akademisyen/subelerim',                  title: 'Şubelerim',            desc: 'Verdiğim dersler', icon: '📚' },
		{ href: '/obs/akademisyen/not-girisi',                 title: 'Not Girişi',            desc: 'Öğrenci notlarını gir',  icon: '📝' },
		{ href: '/obs/akademisyen/yoklama-girisi',             title: 'Yoklama Girişi',        desc: 'Haftalık yoklama',       icon: '✅' },
		{ href: '/obs/akademisyen/danismanlik-ogrencilerim',   title: 'Danışmanlık',           desc: 'Öğrenci listesi',        icon: '🎓' },
		{ href: '/obs/akademisyen/onay-talepleri',             title: 'Onay Talepleri',        desc: `${pendingApprovals} bekliyor`, icon: '⏳' },
...
<ObsShell activePath="/obs/akademisyen" role="akademisyen" termLabel={activeTerm?.name ?? '2025-2026 Bahar'}>
	<span slot="userline">{$user?.name ?? 'Akademisyen'} • Akademisyen Paneli</span>

	<!-- Özet kartlar -->
	<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
		{#each [
			{ label: 'Aktif Dönem',      value: activeTerm?.name ?? (loading ? '…' : '—'),    badge: activeTerm ? 'Aktif' : '' },
			{ label: 'Şube Sayısı',      value: sections.length.toString(),                    badge: '' },
			{ label: 'Bekleyen Onay',    value: pendingApprovals.toString(),                   badge: pendingApprovals > 0 ? 'Var' : '' },
			{ label: 'Ünvan',            value: $user?.name ?? '—',                            badge: '' },
		] as card}
			<div class="rounded-xl border border-black/10 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/5">
				...
				<div class="mt-1.5 truncate text-sm font-bold text-slate-900 dark:text-slate-100">{card.value}</div>
				{#if card.badge}
					<span class="mt-1 inline-block rounded-full {card.badge === 'Var' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300'} px-2 py-0.5 text-[10px] font-bold">{card.badge}</span>
				{/if}
			</div>
		{/each}
	</div>
	...
	<!-- Şubeler özet -->
	{#if sections.length}
		<div class="mt-5 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
			<div class="px-5 py-3 text-xs font-bold tracking-widest text-slate-400 dark:text-slate-500">AKTİF ŞUBELERİM</div>
			{#each sections.slice(0,5) as s}
				<a href="/obs/akademisyen/subelerim"
		...
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
			{#each quickLinks as lnk}
				<a href={lnk.href}
					...
					<div class="min-w-0">
						<div class="text-sm font-semibold group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">{lnk.title}</div>
