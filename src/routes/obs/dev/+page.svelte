<script lang="ts">
	...
	async function clearMyEnrollments() {
		if (
			!browser ||
			!confirm(
				'Oturumdaki öğrenci hesabının TÜM ders kayıtları, notları ve yoklamaları silinsin mi?'
			)
		)
			return;
		clearing = true;
...
<div
	class="flex min-h-screen flex-col items-center justify-center gap-6 bg-gradient-to-br from-slate-100 to-slate-200 p-6 dark:from-slate-950 dark:to-slate-900"
>
	<!-- Başlık -->
	<div class="text-center">
		...
		{#if currentEmail}
			<div
				class="mt-2 inline-block rounded-full bg-slate-200 px-3 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300"
			>
	...
	{#if err}
		<div
			class="w-full max-w-md rounded-xl bg-red-50 px-4 py-3 text-sm text-red-600 dark:bg-red-950/40 dark:text-red-400"
		>
			{err}
		</div>
	{/if}
	{#if clearMsg}
		<div
			class="w-full max-w-md rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200"
		>
			{clearMsg}
		</div>
	{/if}
	...
	<div
		class="w-full max-w-md rounded-2xl border border-amber-200 bg-amber-50/80 p-4 dark:border-amber-900/50 dark:bg-amber-950/20"
	>
		<div class="text-sm font-bold text-amber-900 dark:text-amber-200">
			Öğrenci — demo ders kayıtlarını temizle
		</div>
		<p class="mt-1 text-xs text-amber-800/90 dark:text-amber-300/90">
			Alınan dersler, sınav takvimi ve ders ekle/bırak listesi veritabanındaki kayıtlardan gelir.
			Yerel mock yoktur; temizlemek için:
		</p>
		<button
			type="button"
			disabled={clearing}
	...
	<div class="grid w-full max-w-md grid-cols-1 gap-3 sm:grid-cols-3">
		{#each ['ogrenci', 'Akademisyen', 'Admin'] as role}
			<button
				...
				type="button"
				class="group flex flex-col items-center gap-2 rounded-2xl bg-gradient-to-b {ROLE_COLORS[
					role
				]} p-5 text-white shadow-lg transition hover:scale-105 hover:shadow-xl active:scale-95
	...
	<!-- Test hesapları -->
	<div
		class="w-full max-w-md rounded-2xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5"
	>
		<div
					...
					<div class="flex items-center justify-between px-5 py-3">
						<div>
							<div class="flex items-center gap-2">
								...
								{#if acc.existed}
									<span
										class="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-500 dark:bg-white/10 dark:text-slate-400"
										>Mevcut</span
									>
								{:else}
									<span
										class="rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400"
										>Yeni</span
									>
								{/if}
							</div>
						...
						<div class="text-right">
							<div class="font-mono text-xs font-bold text-sky-600 dark:text-sky-400">
								{acc.password}
							</div>
							<button
								on:click={() => switchRole(acc.obs_role)}
								type="button"
								class="mt-1 rounded-lg bg-gradient-to-r {ROLE_COLORS[acc.obs_role] ??
									'from-slate-500 to-slate-600'} px-2.5 py-1 text-[10px] font-bold text-white transition hover:opacity-90"
							>
								Geç →
							</button>
						</div>
