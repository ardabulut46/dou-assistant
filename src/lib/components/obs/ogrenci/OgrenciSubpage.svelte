<script lang="ts">
	...
	let profileForm   = { full_name: '', phone: '', address: '', emergency_contact: '', emergency_phone: '' };
	...
	let composeForm = { subject: '', body: '', receiver_name: 'Danışmanım', receiver_type: 'akademisyen' };
	...
	async function saveProfile() {
		profileSaving = true; profileSaved = false; profileError = null;
		const token = localStorage.token ?? null;
		try {
	...
	async function finalizeDrop() {
		...
		dropSubmitting = true; dropSuccess = null; dropError = null;
		const token = localStorage.token ?? null;
		try {
		await Promise.all(
	...
	async function submitDocRequest() {
		...
		} finally { docSubmitting = false; }
	}

	async function sendMessage() {
		composeSending = true; composeSuccess = null; composeError = null;
		const token = localStorage.token ?? null;
		try {
			await sendDouMessageApi(token, {
			...
			composeForm = { subject: '', body: '', receiver_name: 'Danışmanım', receiver_type: 'akademisyen' };
			showCompose = false;
			const r = await getDouSent(token);
			sentMsgs = r.messages;
		} catch (e: unknown) {
			composeError = e instanceof Error ? e.message : 'Hata.';
		} finally { composeSending = false; }
	}

	function changePassword() {
		pwBusy = true; pwErr = null; pwOk = null;
		setTimeout(() => {
			if (!pwForm.current_password || !pwForm.new_password) { pwErr = 'Tüm alanlar zorunludur.'; }
			...
			else if (!/[A-Z]/.test(pwForm.new_password)) { pwErr = 'En az 1 büyük harf içermeli.'; }
			else if (!/\d/.test(pwForm.new_password)) { pwErr = 'En az 1 rakam içermeli.'; }
			else { pwOk = 'Şifreniz başarıyla güncellendi.'; pwForm = { current_password: '', new_password: '', confirm_password: '' }; }
			pwBusy = false;
		}, 400);
	}
...
<div class="space-y-4">
		...
		{:else if loadErr}
			<div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-900/40 dark:bg-amber-950/30">
				...
				<div class="mt-2 text-[11px] text-amber-600 dark:text-amber-300">Backend yeniden başlatılması gerekiyor olabilir.</div>
			</div>
	...
	{:else if apiKey === 'profile' && profile}
		{#if !profileEdit}
			<!-- ── Hero kartı ────────────────────────────────────── -->
			<div class="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
				<!-- Gradient banner -->
				<div class="h-24 bg-gradient-to-r from-sky-500 via-indigo-500 to-violet-500"></div>

				<div class="relative px-6 pb-6">
					<!-- Avatar -->
					<div class="absolute -top-8 left-6 flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-400 to-indigo-600 text-2xl font-black text-white shadow-lg ring-4 ring-white dark:ring-slate-900">
					...
					<div class="flex items-start justify-between pt-10">
						<div>
							<div class="text-xl font-bold text-slate-900 dark:text-slate-100">{profile.full_name ?? profile.email}</div>
							<div class="mt-0.5 text-sm text-slate-500">{profile.student_no} · {profile.faculty_name ?? profile.department_name}</div>
							<div class="mt-0.5 text-xs text-slate-400">{profile.department_name} — {profile.program ?? 'Lisans'} · {profile.class_level}. Sınıf</div>
						</div>
						<button
							on:click={() => { profileEdit = true; profileSaved = false;
								profileForm = {
							...
							<svg class="h-3.5 w-3.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
							Düzenle
						</button>
					</div>
				...
				{#if true}
					...
					{@const tamamAkts = ((profile as unknown as Record<string,number>).completed_akts   ?? 0)}
					{@const toplamAkts= ((profile as unknown as Record<string,number>).total_akts_required ?? 120)}
					<div class="mt-5 grid grid-cols-4 gap-3">
						{#each [
							{ lbl: 'AGNO',  val: agno.toFixed(2),       sub: 'Kümülatif',      color: agno >= 3.0 ? 'text-emerald-600 dark:text-emerald-400' : agno >= 2.0 ? 'text-sky-600 dark:text-sky-400' : 'text-red-500' },
							{ lbl: 'DNO',   val: dno.toFixed(2),        sub: 'Bu Dönem',       color: dno  >= 3.0 ? 'text-emerald-600 dark:text-emerald-400' : dno  >= 2.0 ? 'text-sky-600 dark:text-sky-400' : 'text-red-500' },
							{ lbl: 'AKTS',  val: tamamAkts.toString(),   sub: `/ ${toplamAkts}`, color: 'text-slate-800 dark:text-slate-100' },
							{ lbl: 'Durum', val: profile.status === 'active' ? 'Aktif' : (profile.status ?? 'Aktif'),
							                sub: profile.is_financially_eligible ? '✓ Mali Uygun' : '✗ Borç Var',
							                color: profile.status === 'active' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500' },
						] as st}
							<div class="rounded-xl border border-black/8 bg-slate-50 px-4 py-3 dark:border-white/8 dark:bg-white/5">
								<div class="text-[10px] font-bold uppercase tracking-widest text-slate-400">{st.lbl}</div>
				...
				{/if}
					...
					<!-- İletişim bilgileri -->
					<div class="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-black/8 bg-black/5 dark:border-white/8 dark:bg-white/10 sm:grid-cols-3">
						{#each [
							...
							<div class="bg-white px-4 py-3 dark:bg-slate-900">
								<div class="text-[10px] font-semibold uppercase tracking-wider text-slate-400">{lbl}</div>
								<div class="mt-0.5 truncate text-sm font-medium text-slate-800 dark:text-slate-100">{val}</div>
							</div>
						{/each}
					</div>

					{#if profileSaved}
						<div class="mt-4 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">✓ Profil güncellendi.</div>
					{/if}
				</div>
			</div>
		{:else}
			...
			<div class="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
				<div class="border-b border-black/5 px-6 py-4 dark:border-white/10">
					<div class="text-sm font-bold text-slate-700 dark:text-slate-200">Kişisel Bilgileri Düzenle</div>
					<div class="text-xs text-slate-400">Öğrenci no, bölüm ve program bilgileri değiştirilemez.</div>
				</div>
				<div class="px-6 py-5">
					{#if profileError}
						<div class="mb-4 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{profileError}</div>
					{/if}
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
						{#each [
							{ lbl: 'Ad Soyad',           field: 'full_name',         type: 'text', ph: 'Adınız ve soyadınız' },
							{ lbl: 'Cep Telefonu',        field: 'phone',             type: 'tel',  ph: '0532 XXX XX XX' },
							{ lbl: 'Adres',               field: 'address',           type: 'text', ph: 'İkamet adresiniz' },
							{ lbl: 'Acil İrtibat (Ad)',   field: 'emergency_contact', type: 'text', ph: 'Anne / Baba / Eş' },
							{ lbl: 'Acil İrtibat (Tel)',  field: 'emergency_phone',   type: 'tel',  ph: '0532 XXX XX XX' },
						] as fld}
							<label class="block">
								...
								<input
									...
									type={fld.type} placeholder={fld.ph}
					...
					<div class="mt-5 rounded-xl border border-black/8 bg-slate-50 p-4 dark:border-white/8 dark:bg-white/5">
						<div class="mb-3 text-[10px] font-bold uppercase tracking-widest text-slate-400">Değiştirilemeyen Bilgiler</div>
						<div class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
							{#each [['Öğrenci No', profile.student_no], ['Bölüm', profile.department_name], ['Program', profile.program ?? 'Lisans']] as [l,v]}
								<div><span class="text-xs text-slate-400">{l}</span><div class="font-medium text-slate-600 dark:text-slate-300">{v}</div></div>
							{/each}
						</div>
					</div>

					<div class="mt-5 flex gap-3">
						<button on:click={saveProfile} disabled={profileSaving} type="button"
							...
							{#if profileSaving}<div class="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></div>{/if}
							{profileSaving ? 'Kaydediliyor…' : 'Kaydet'}
						</button>
						<button on:click={() => { profileEdit = false; profileError = null; }} type="button"
		...
		{:else if apiKey === 'terms' && terms.length}
			<div class="overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
				...
				{#each terms as t}
					<div class="grid grid-cols-5 border-t border-black/5 px-5 py-3.5 text-sm dark:border-white/10 {t.is_active ? 'bg-sky-50/40 dark:bg-sky-900/10' : ''}">
						<div class="col-span-2 font-medium">
							{t.name}
							{#if t.is_active}<span class="ml-2 rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-bold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">Aktif</span>{/if}
						</div>
		...
		{:else if apiKey === 'advisor' && advisor?.advisor}
			<div class="rounded-xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
				<div class="flex items-start gap-5">
					<div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-400 to-indigo-600 text-2xl font-bold text-white shadow">
						{advisor.advisor.name.charAt(0)}
					</div>
					<div class="flex-1">
						<div class="text-xl font-bold">{advisor.advisor.name}</div>
						{#if advisor.advisor.title}<div class="mt-0.5 text-sm text-slate-500">{advisor.advisor.title}</div>{/if}
					</div>
				</div>
				<div class="mt-5 grid grid-cols-2 gap-3">
					{#each [['E-posta', advisor.advisor.email, true],['Oda', advisor.advisor.office ?? '—', false],['Telefon', advisor.advisor.phone ?? '—', false],['Bölüm', advisor.advisor.department_name ?? '—', false]] as [lbl, val, isEmail]}
						<div class="rounded-xl border border-black/5 bg-slate-50 px-4 py-3 dark:border-white/5 dark:bg-white/5">
							<div class="text-xs font-semibold text-slate-400">{lbl}</div>
							{#if isEmail}
								<a href="mailto:{val}" class="mt-1 block text-sm font-medium text-sky-600 hover:underline dark:text-sky-400">{val}</a>
							{:else}
				...
				<div class="mt-4 flex gap-2">
					<button on:click={() => showCompose = true} type="button"
		...
		{:else if apiKey === 'enrollments'}
			<div class="mb-2 flex items-center justify-between rounded-xl border border-black/10 bg-white px-5 py-3 shadow-sm dark:border-white/10 dark:bg-white/5">
				<div class="flex items-center gap-3">
					<span class="text-sm font-semibold">{enrollments.length} ders</span>
					<span class="rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-bold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">{totalAkts} AKTS</span>
				</div>
			</div>
			<div class="overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					<table class="w-full text-sm">
						...
						<tbody>
							{#each enrollments as e}
								<tr class="border-t border-black/5 hover:bg-slate-50/50 transition-colors dark:border-white/10 dark:hover:bg-white/5">
									<td class="px-4 py-3 font-mono text-xs font-semibold text-slate-600 dark:text-slate-300">{e.course_code}</td>
							...
							{:else}
								<tr><td colspan="7" class="px-4 py-8 text-center text-sm text-slate-400">Kayıtlı ders yok.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
		...
		{:else if apiKey === 'ders-kayit'}
			...
			<div class="rounded-xl border border-sky-200 bg-sky-50 px-5 py-4 dark:border-sky-900/40 dark:bg-sky-950/20">
				...
				<div class="mt-2 h-2 w-full overflow-hidden rounded-full bg-sky-100 dark:bg-sky-900/40">
					<div class="h-2 rounded-full transition-all
						{(totalAkts + cartAkts) / AKTS_LIMIT > 0.9 ? 'bg-red-500' : (totalAkts + cartAkts) / AKTS_LIMIT > 0.7 ? 'bg-amber-400' : 'bg-sky-500'}"
			...
			<!-- Sepet / Kesinleştir -->
			{#if cart.length}
				<div class="rounded-xl border border-emerald-200 bg-emerald-50/80 px-5 py-4 dark:border-emerald-900/40 dark:bg-emerald-950/20">
					<div class="mb-3 flex items-center justify-between">
						<span class="font-semibold text-emerald-800 dark:text-emerald-200">Sepet ({cart.length} ders · {cartAkts} AKTS)</span>
						<button on:click={() => cart = []} type="button" class="text-xs text-emerald-600 hover:text-emerald-800 dark:text-emerald-400">Temizle</button>
					</div>
					<div class="mb-3 space-y-1.5">
						{#each cart as c}
							<div class="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2 dark:bg-white/5">
								<span class="font-mono text-xs font-semibold text-slate-500 mr-2">{c.course_code}</span>
								<span class="flex-1 text-sm font-medium">{c.course_name}</span>
								<span class="mx-3 text-xs text-slate-400">{c.akts} AKTS</span>
								<button on:click={() => toggleCart(c)} type="button" class="text-red-400 hover:text-red-600 text-sm">✕</button>
							</div>
						{/each}
					</div>
					{#if enrollSuccess}
						<div class="mb-3 rounded-lg bg-emerald-100 px-3 py-2 text-sm text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200">{enrollSuccess}</div>
					{/if}
					{#if enrollError}
						<div class="mb-3 rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 dark:bg-red-900/40 dark:text-red-200">{enrollError}</div>
					{/if}
					<button on:click={finalizeEnrollment} disabled={enrollSubmitting} type="button"
						...
						{/if}
						{enrollSubmitting ? 'Gönderiliyor…' : '✓ Kayıt İsteğini Kesinleştir → Akademisyene Gönder'}
					</button>
				</div>
			{:else if enrollSuccess}
				<div class="rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-800 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-200">
					{enrollSuccess}
				</div>
			{/if}
					...
					<table class="w-full text-sm">
						...
						<tbody>
							{#each availableCourses as c}
								...
								<tr class="border-t border-black/5 dark:border-white/10 {inCart ? 'bg-sky-50/50 dark:bg-sky-900/10' : 'hover:bg-slate-50/50 dark:hover:bg-white/5'} transition-colors">
									<td class="px-4 py-3 font-mono text-xs font-semibold text-slate-500">{c.course_code}</td>
									...
									<td class="px-4 py-3 text-center text-xs {full ? 'text-red-500' : 'text-slate-500'}">
										{c.enrolled}/{c.capacity}
										{#if full}<span class="ml-1 rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-bold text-red-700 dark:bg-red-900/40 dark:text-red-300">Dolu</span>{/if}
									</td>
							...
							{:else}
								<tr><td colspan="7" class="px-4 py-8 text-center text-sm text-slate-400">Açılan ders bulunamadı.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
		...
		{:else if apiKey === 'ders-ekle'}
			<div class="rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
				<div class="flex items-center justify-between border-b border-black/5 px-5 py-3 dark:border-white/10">
					<div>
					...
					{#if markedDrop.size > 0}
						<div class="flex items-center gap-2">
							...
							<button on:click={finalizeDrop} disabled={dropSubmitting} type="button"
				...
				{#if dropSuccess}
					<div class="mx-5 mt-3 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{dropSuccess}</div>
				{/if}
				{#if dropError}
					<div class="mx-5 mt-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{dropError}</div>
				{/if}
				...
				<div class="overflow-x-auto">
					<table class="w-full text-sm">
						...
						{#each enrollments as e}
							...
							<tr class="border-t border-black/5 dark:border-white/10 transition-colors
								{dropped ? 'bg-slate-50/80 opacity-60 dark:bg-white/5'
								 : marked ? 'bg-red-50/60 dark:bg-red-950/20'
								 : 'hover:bg-slate-50/50 dark:hover:bg-white/5'}">
								<td class="px-4 py-3">
									{#if dropped}
										<span title="Bırakma isteği gönderildi" class="text-slate-400 text-sm">⏳</span>
									{:else}
								...
								<td class="px-4 py-3 font-mono text-xs font-semibold {marked ? 'text-red-500' : dropped ? 'text-slate-400' : 'text-slate-500'}">{e.course_code}</td>
								<td class="px-4 py-3 font-medium {dropped ? 'line-through text-slate-400' : marked ? 'line-through text-slate-400' : ''}">{e.course_name}</td>
								<td class="px-4 py-3 text-center {dropped ? 'text-slate-400' : ''}">{e.akts}</td>
								<td class="px-4 py-3 text-xs {dropped ? 'text-slate-400' : 'text-slate-500'}">{e.instructor_name ?? '—'}</td>
								<td class="px-4 py-3 text-xs {dropped ? 'text-slate-400' : ''}">{e.day_of_week ?? '—'} {e.start_time ?? ''}
									{#if dropped}<span class="ml-1 rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-bold text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">Beklemede</span>{/if}
								</td>
							</tr>
							{:else}
								<tr><td colspan="6" class="px-4 py-8 text-center text-sm text-slate-400">Kayıtlı ders yok.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
				{#if markedDrop.size > 0}
					<div class="border-t border-black/5 bg-red-50/40 px-5 py-3 text-xs text-red-600 dark:border-white/10 dark:bg-red-950/10 dark:text-red-400">
						İşaretlenen dersler için bırakma talebi akademisyeninize iletilecek. Onaylandıktan sonra kaydınız silinecektir.
					</div>
				{/if}
			</div>
			...
			<div class="overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
				<div class="overflow-x-auto">
					<table class="w-full text-sm">
						...
						<tbody>
							{#each exams as ex}
								<tr class="border-t border-black/5 dark:border-white/10 hover:bg-slate-50/50 transition-colors">
									<td class="px-4 py-3 font-mono text-xs font-semibold">{ex.course_code}</td>
									<td class="px-4 py-3">
										<span class="rounded-full px-2 py-0.5 text-xs font-medium
											{ex.exam_type === 'midterm' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300' : ex.exam_type === 'final' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-slate-100 text-slate-600'}">
							...
							{:else}
								<tr><td colspan="6" class="px-4 py-8 text-center text-sm text-slate-400">Sınav bulunamadı.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
		...
		{:else if apiKey === 'schedule'}
			...
			<div class="space-y-3">
				{#each DAYS as day}
					...
					{#if rows.length}
						<div class="overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
							<div class="bg-slate-100/70 px-5 py-2 text-xs font-bold text-slate-500 dark:bg-white/5 dark:text-slate-400">{day}</div>
							{#each rows as s}
								<div class="flex items-center gap-4 border-t border-black/5 px-5 py-3 dark:border-white/10">
									<span class="w-28 shrink-0 font-mono text-xs text-slate-400">{s.start}–{s.end}</span>
									<span class="rounded-lg bg-sky-100 px-2 py-0.5 font-mono text-xs font-bold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">{s.course_code}</span>
				...
				{/each}
				{#if !schedule.length}
					<div class="rounded-xl border border-dashed border-black/15 p-8 text-center text-sm text-slate-400">Ders programı yüklenemedi.</div>
				{/if}
			</div>
				...
				<div class="overflow-x-auto">
					<table class="w-full text-sm">
						...
						<tbody>
							{#each grades as g}
								<tr class="border-t border-black/5 dark:border-white/10 hover:bg-slate-50/50 transition-colors">
									<td class="px-4 py-3 font-mono text-xs font-semibold text-slate-600 dark:text-slate-300">{g.course_code}</td>
									<td class="px-4 py-3 text-center">{g.midterm ?? '—'}</td>
									<td class="px-4 py-3 text-center">{g.final ?? '—'}</td>
									<td class="px-4 py-3 text-center">
										{#if g.is_published && g.letter_grade}
											<span class="rounded-full px-2.5 py-0.5 text-xs font-bold {GRADE_COLOR[g.letter_grade] ?? 'bg-slate-100 text-slate-600'}">{g.letter_grade}</span>
										{:else}<span class="text-slate-300">—</span>{/if}
									</td>
									<td class="px-4 py-3 text-center text-xs">
										{#if g.is_published}<span class="text-emerald-600 dark:text-emerald-400">Yayınlandı</span>
										{:else if g.is_finalized}<span class="text-amber-600 dark:text-amber-400">Kesinleşti</span>
										{:else}<span class="text-slate-400">Bekleniyor</span>{/if}
									</td>
								</tr>
							{:else}
								<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-slate-400">Not bulunamadı.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
		...
		{:else if apiKey === 'gpa'}
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
				<div class="flex flex-col items-center justify-center rounded-xl border border-black/10 bg-white p-8 shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="text-xs font-bold tracking-wider text-slate-400">KÜMÜLATİF GNO</div>
					<div class="mt-3 text-5xl font-black text-sky-600 dark:text-sky-400">{cumulativeGpa?.toFixed(2) ?? '—'}</div>
					<div class="mt-2 text-xs text-slate-400">4.00 üzerinden</div>
				</div>
				<div class="col-span-2 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="grid grid-cols-3 bg-slate-50 px-5 py-3 text-xs font-bold text-slate-500 dark:bg-white/5 dark:text-slate-400">
					...
					{#each (gpaTerms as {term_name:string;term_gpa:number|null;akts_completed:number}[]) as t}
						<div class="grid grid-cols-3 border-t border-black/5 px-5 py-3.5 text-sm dark:border-white/10">
							<div class="font-medium">{t.term_name}</div>
							<div class="text-center font-bold {t.term_gpa && t.term_gpa >= 2.0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}">
	...
	{:else if apiKey === 'transcript' && transcript}
		<!-- Resmi başlık -->
		<div class="mb-4 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5 print:shadow-none print:border-0">
			<div class="border-b border-black/10 bg-slate-50 px-6 py-4 dark:border-white/10 dark:bg-white/5 print:bg-white">
				<div class="flex items-start justify-between gap-4">
					<div>
						<div class="text-xs font-bold uppercase tracking-widest text-slate-400">Doğuş Üniversitesi</div>
						<div class="mt-1 text-lg font-bold text-slate-900 dark:text-slate-100">Akademik Transkript</div>
						<div class="mt-1 text-sm text-slate-500">{profile?.full_name ?? $user?.name ?? '—'} · No: {profile?.student_no ?? '—'}</div>
						<div class="text-xs text-slate-400">{profile?.department ?? '—'} · {profile?.program ?? 'Lisans'}</div>
					</div>
					<div class="flex items-center gap-5 shrink-0">
						<div class="text-center">
							<div class="text-xs font-semibold text-slate-400">Kümülatif GNO</div>
							<div class="mt-1 text-3xl font-black {(cumulativeGpa ?? 0) >= 3.0 ? 'text-emerald-600 dark:text-emerald-400' : (cumulativeGpa ?? 0) >= 2.0 ? 'text-sky-600 dark:text-sky-400' : 'text-red-500'}">{cumulativeGpa?.toFixed(2) ?? '—'}</div>
						</div>
						<div class="text-center">
							<div class="text-xs font-semibold text-slate-400">Toplam AKTS</div>
							<div class="mt-1 text-3xl font-black text-slate-700 dark:text-slate-200">{transcriptTotalAkts ?? '—'}</div>
						</div>
						<button on:click={() => window.print()} type="button"
		...
		{#each (transcript as {term_name:string;term_gpa:number;term_akts:number;courses:{code:string;name:string;credits:number;akts:number;letter:string;grade_point:number}[]}[]) as term}
			<div class="mb-3 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
				<!-- Dönem başlığı -->
				<div class="flex items-center justify-between bg-slate-100 px-5 py-3 dark:bg-white/10">
					...
					<div class="flex gap-4 text-xs text-slate-500">
						<span>Dönem AKTS: <strong class="text-slate-700 dark:text-slate-200">{term.term_akts}</strong></span>
						<span>DNO: <strong class="{term.term_gpa >= 3.0 ? 'text-emerald-600 dark:text-emerald-400' : term.term_gpa >= 2.0 ? 'text-sky-600 dark:text-sky-400' : 'text-red-500'}">{term.term_gpa.toFixed(2)}</strong></span>
					</div>
				</div>
				<!-- Ders satırları -->
				<div class="overflow-x-auto">
						...
						<tbody>
							{#each term.courses as c}
								<tr class="border-t border-black/5 dark:border-white/10">
									<td class="px-5 py-2.5 font-mono text-xs font-semibold text-slate-500">{c.code}</td>
									...
									<td class="px-5 py-2.5 text-center text-xs text-slate-500">{c.grade_point.toFixed(2)}</td>
									<td class="px-5 py-2.5 text-center">
										<span class="rounded-full px-2.5 py-0.5 text-xs font-bold {GRADE_COLOR[c.letter] ?? 'bg-slate-100 text-slate-600'}">{c.letter}</span>
									</td>
								</tr>
							{/each}
						</tbody>
			...
			<div class="mb-4 overflow-hidden rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
				<div class="flex items-center justify-between gap-4">
					...
					<div class="text-right">
						<div class="text-xs font-semibold text-slate-500 dark:text-slate-400">Genel İlerleme</div>
						<div class="mt-1 text-2xl font-bold text-sky-600 dark:text-sky-400">%{(curriculum.overall_progress_pct ?? 0).toFixed(1)}</div>
					</div>
				</div>
			...
			{#each (curriculum.categories ?? []) as cat}
				...
				<div class="mb-4 overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
					<div class="flex items-center justify-between bg-slate-50 px-5 py-3 dark:bg-white/5">
						...
						<div class="text-xs text-slate-500">{done}/{total} tamamlandı
							{#if ongoing > 0}<span class="ml-1.5 rounded-full bg-sky-100 px-1.5 py-0.5 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">{ongoing} devam</span>{/if}
						</div>
					</div>
					{#each cat.courses as c}
						...
						{@const statusLabel = { tamamlandi: 'Tamamlandı', devam_ediyor: 'Devam Ediyor', alinmadi: 'Alınmadı' }}
						<div class="grid grid-cols-12 items-center border-t border-black/5 px-5 py-2.5 text-sm dark:border-white/10">
							<div class="col-span-2 font-mono text-xs font-semibold text-slate-500">{c.code}</div>
							<div class="col-span-6 font-medium">{c.name}</div>
							<div class="col-span-1 text-center text-xs text-slate-400">{c.akts} AKTS</div>
							<div class="col-span-2 text-center">
								{#if c.grade}
									<span class="rounded-full px-2 py-0.5 text-xs font-bold {GRADE_COLOR[c.grade] ?? 'bg-slate-100 text-slate-600'}">{c.grade}</span>
								{:else}
									<span class="text-slate-300">—</span>
								{/if}
							</div>
							<div class="col-span-1 text-right">
								<span class="rounded-full px-2 py-0.5 text-[10px] font-semibold {statusColors[c.status as keyof typeof statusColors] ?? statusColors.alinmadi}">
	...
	{:else if apiKey === 'attendance'}
						...
						<tbody>
							{#each attendance as a}
								<tr class="border-t border-black/5 dark:border-white/10">
									...
									<td class="px-4 py-3 text-center font-semibold {a.absent_count > 3 ? 'text-red-500' : ''}">{a.absent_count}</td>
									<td class="px-4 py-3 text-center">
										<div class="font-bold {a.attendance_pct >= 70 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}">%{a.attendance_pct.toFixed(1)}</div>
										<div class="mt-1 h-1.5 w-16 mx-auto overflow-hidden rounded-full bg-slate-200 dark:bg-white/10">
											<div class="h-full rounded-full transition-all {a.attendance_pct >= 70 ? 'bg-emerald-500' : 'bg-red-500'}" style="width:{a.attendance_pct}%"></div>
										</div>
									</td>
									<td class="px-4 py-3 text-center">
										<span class="rounded-full px-2.5 py-0.5 text-xs font-medium
											{a.status === 'ok' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : a.status === 'warning' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'}">
											{a.status === 'ok' ? 'İyi' : a.status === 'warning' ? 'Uyarı' : 'Kritik'}
										</span>
									</td>
								</tr>
							{:else}
								<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-slate-400">Devamsızlık kaydı yok.</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
		...
		{:else if apiKey === 'announcements'}
			...
			{:else}
				<div class="rounded-xl border border-dashed border-black/15 p-8 text-center text-sm text-slate-400">Duyuru yok.</div>
			{/if}
		...
		{:else if apiKey === 'inbox' || apiKey === 'sent'}
			...
			<div class="space-y-2">
				{#each msgs as m}
					<div class="rounded-xl border bg-white p-5 shadow-sm dark:bg-white/5
						{apiKey === 'inbox' && !m.is_read ? 'border-sky-200 dark:border-sky-800' : 'border-black/10 dark:border-white/10'}">
						<div class="flex items-start justify-between gap-3">
							<div class="font-semibold text-sm {apiKey === 'inbox' && !m.is_read ? 'text-sky-700 dark:text-sky-300' : ''}">{m.subject}</div>
							{#if apiKey === 'inbox' && !m.is_read}
								<span class="shrink-0 rounded-full bg-sky-100 px-1.5 py-0.5 text-[10px] font-bold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">YENİ</span>
							{/if}
						</div>
				...
				{:else}
					<div class="rounded-xl border border-dashed border-black/15 p-8 text-center text-sm text-slate-400 dark:border-white/15">
			...
			{#if showCompose}
				<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
					<div class="w-full max-w-lg rounded-2xl border border-black/10 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-slate-900">
						<div class="mb-5 flex items-center justify-between">
							<div class="font-bold text-slate-800 dark:text-slate-100">Yeni Mesaj</div>
							<button on:click={() => showCompose = false} type="button" class="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">✕</button>
						</div>
						{#if composeError}
							<div class="mb-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{composeError}</div>
						{/if}
						<div class="space-y-3">
							<label class="block">
								<div class="mb-1 text-xs font-semibold text-slate-500">Alıcı</div>
								<input bind:value={composeForm.receiver_name} placeholder="Danışmanım"
							...
							<label class="block">
								...
								<input bind:value={composeForm.subject} placeholder="Mesaj konusu"
									class="w-full rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-400/30 dark:border-white/10 dark:bg-slate-800" />
							</label>
							<label class="block">
								<div class="mb-1 text-xs font-semibold text-slate-500">Mesaj</div>
								<textarea bind:value={composeForm.body} rows="6" placeholder="Mesajınızı yazın…"
									class="w-full rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-400/30 dark:border-white/10 dark:bg-slate-800 resize-none"></textarea>
							</label>
							<div class="flex gap-2 pt-1">
								<button on:click={sendMessage} disabled={composeSending || !composeForm.subject || !composeForm.body} type="button"
								...
								<button on:click={() => showCompose = false} type="button"
		...
		{:else if apiKey === 'doc-request'}
			<div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
				<div class="rounded-xl border border-black/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
					...
					{#if docSuccess}<div class="mb-3 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{docSuccess}</div>{/if}
					{#if docError}<div class="mb-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{docError}</div>{/if}
					<div class="space-y-4">
						<label class="block">
							<div class="mb-1.5 text-xs font-semibold text-slate-500">Talep Eden Kurum</div>
							<input bind:value={docForm.requesting_institution} placeholder="Kurum adı"
						...
						<label class="block">
							...
							<input bind:value={docForm.request_reason} placeholder="Neden gerekiyor?"
								class="w-full rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-400/20 dark:border-white/10 dark:bg-white/5" />
						</label>
						<label class="block">
							<div class="mb-1.5 text-xs font-semibold text-slate-500">Belge Türü</div>
							<select bind:value={docForm.document_type}
						...
						<label class="block">
							...
							<select bind:value={docForm.document_subtype}
								class="w-full rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm outline-none dark:border-white/10 dark:bg-white/5">
								{#each docSubtypes as sub}<option value={sub}>{sub}</option>{/each}
							</select>
						</label>
						<button on:click={submitDocRequest} disabled={docSubmitting} type="button"
						...
						<div class="space-y-2">
							{#each docRequests as req}
								<div class="rounded-xl border border-black/5 p-4 dark:border-white/10">
									<div class="flex items-center justify-between">
										<span class="font-medium text-sm">{req.document_type} — {req.document_subtype}</span>
										<span class="rounded-full px-2 py-0.5 text-xs {req.status === 'tamamlandı' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300'}">
											{req.status}
										</span>
									</div>
									<div class="mt-1 text-xs text-slate-400">{req.requesting_institution} · {req.created_at?.slice(0,10)}</div>
								</div>
							{/each}
						</div>
					{:else}
		...
		{:else if apiKey === 'change-pw'}
			<div class="mx-auto max-w-md rounded-xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
				...
				<p class="mb-5 text-xs text-slate-400">Kurallar: en az 8 karakter, 1 büyük harf, 1 rakam.</p>
				{#if pwOk}<div class="mb-4 rounded-lg bg-emerald-50 px-3 py-2.5 text-sm text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{pwOk}</div>{/if}
				{#if pwErr}<div class="mb-4 rounded-lg bg-red-50 px-3 py-2.5 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{pwErr}</div>{/if}
				<div class="space-y-4">
					{#each [['Mevcut Şifre','current_password'],['Yeni Şifre','new_password'],['Yeni Şifre (Tekrar)','confirm_password']] as [lbl,field]}
						<label class="block">
							...
							<input bind:value={pwForm[field as keyof typeof pwForm]} type="password" autocomplete="off"
								class="w-full rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-400/20 dark:border-white/10 dark:bg-white/5" />
						</label>
					{/each}
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
			<div class="w-full max-w-lg rounded-2xl border border-black/10 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-slate-900">
				...
				<div class="space-y-3">
					...
					<label class="block">
						<div class="mb-1 text-xs font-semibold text-slate-500">Mesaj</div>
						<textarea bind:value={composeForm.body} rows="5" placeholder="Mesajınızı yazın…"
							class="w-full resize-none rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-400/30 dark:border-white/10 dark:bg-slate-800"></textarea>
					</label>
					{#if composeSuccess}<div class="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{composeSuccess}</div>{/if}
					{#if composeError}<div class="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{composeError}</div>{/if}
					<div class="flex gap-2 pt-1">
						<button on:click={sendMessage} disabled={composeSending || !composeForm.subject || !composeForm.body} type="button"
							class="flex-1 rounded-xl bg-sky-500 py-2.5 text-sm font-semibold text-white hover:bg-sky-400 disabled:opacity-50 transition-colors">
							{composeSending ? 'Gönderiliyor…' : 'Gönder'}
