...
_COURSES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "department_id": "dept-3",
        "code": "YBS201",
        "name": "Sistem Analizi ve Tasarım",
        "credits": 3,
        "akts": 5,
        "class_year": 2,
        "type": "Z",
...
_SECTIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "course_id": "crs-5",
        "course_code": "YBS201",
        "course_name": "Sistem Analizi ve Tasarım",
        "section_code": "A",
        "section_no": 1,
        "term_id": "term-2",
        "instructor_id": "mock-acad-1",
...
_CALENDAR_EVENTS: list[dict[str, Any]] = [
    ...
    },
    {
        "id": "cal-3",
        "term_id": "term-2",
        "event_type": "MIDTERM_GRADE_ENTRY",
        "title": "Vize Not Girişi",
    ...
    },
    {
        "id": "cal-4",
        "term_id": "term-2",
        "event_type": "FINAL_GRADE_ENTRY",
        "title": "Final Not Girişi",
    ...
    },
    {
        "id": "cal-5",
        "term_id": "term-2",
        "event_type": "ADVISOR_APPROVAL",
        "title": "Danışman Onay Penceresi",
...
@public_router.post(
    "/dev/seed-users",
    summary="[DEV] Test kullanıcıları oluştur veya döndür",
...
async def term_calendar(term_id: str, _user=Depends(get_verified_user)):
    events = [
        e
        for e in _CALENDAR_EVENTS + _created["calendar_events"]
...
@public_router.post(
    "/messages", status_code=status.HTTP_201_CREATED, summary="Mesaj gönder"
)
    ...
    schedule = [
        {
            ...
            "course_name": "Programlamaya Giriş",
            "classroom": "A-101",
            "instructor": "Dr. Öğr. Üy. Ayşe Yılmaz",
        },
        {
            "day": "Salı",
            "start": "11:00",
...
async def student_me_document_requests(user=Depends(get_verified_user)):
    return {
        "student_user_id": user.id,
        "requests": _DOCUMENT_REQUESTS + _created["document_requests"],
        "_mock": True,
    }
...
class DocumentRequestCreate(BaseModel):
    ...
    document_type: str = Field(
        ..., description="transkript / öğrenci_belgesi / disiplin_belgesi"
    )
    document_subtype: str = Field(
        ..., description="Resmi / Onaysız / Türkçe / İngilizce"
    )
...
@student_router.post(
    ...
    summary="Belge talebi gönder",
)
async def student_create_document_request(
    body: DocumentRequestCreate, user=Depends(get_verified_user)
):
    valid_subs = _VALID_SUBTYPE_MAP.get(body.document_type, [])
    if body.document_subtype not in valid_subs:
        raise HTTPException(
            status_code=400,
            detail=f"{body.document_type} için geçerli alt tipler: {valid_subs}",
        )
    row = {
...
async def student_me_announcements(user=Depends(get_verified_user)):
    result = [
        ...
        if a["is_active"] and a["audience_type"] in ("all", "department")
    ]
    ...
    if not available:
        available = [
            ...
            },
            {
                "id": "sec-a2",
                "course_code": "YBS402",
                "course_name": "Bilgi Güvenliği",
                "credits": 3,
                "akts": 5,
                "instructor_name": "Dr. Mehmet Demir",
                "day_of_week": "Salı",
                "start_time": "11:00",
                "end_time": "14:00",
                "classroom": "A-301",
            ...
            },
            {
                ...
                "credits": 3,
                "akts": 6,
                "instructor_name": "Prof. Ahmet Kaya",
                "day_of_week": "Çarşamba",
                "start_time": "09:00",
                "end_time": "12:00",
                "classroom": "B-101",
            ...
            },
            {
                "id": "sec-a4",
                "course_code": "BLM301",
                "course_name": "Ağ ve İletişim Sistemleri",
                "credits": 3,
                "akts": 5,
                "instructor_name": "Dr. Zeynep Demir",
                "day_of_week": "Perşembe",
                "start_time": "14:00",
                "end_time": "17:00",
                "classroom": "A-201",
            ...
            },
            {
                ...
                "credits": 3,
                "akts": 5,
                "instructor_name": "Öğr. Gör. Can Aydın",
                "day_of_week": "Cuma",
                "start_time": "10:00",
                "end_time": "13:00",
                "classroom": "Lab-3",
...
@student_router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Ders kayıt isteği gönder",
)
async def student_enrollment_request(
    body: EnrollmentRequest, user=Depends(get_verified_user)
):
    requests = []
    for sid in body.section_ids:
        ...
        _APPROVAL_REQUESTS.append(
            {**req, "student_name": getattr(user, "name", "Öğrenci"), "note": body.note}
        )
...
async def student_update_profile(
    body: StudentProfileUpdate, user=Depends(get_verified_user)
):
...
async def student_todo_list(user=Depends(get_verified_user)):
    return {
        ...
        "todos": [
            ...
            {
                "id": "todo-3",
                "title": "Bitirme projesi danışman görüşmesi",
                "category": "proje",
                "due_date": "2026-04-22",
                "is_done": False,
                "priority": "medium",
            ...
            {
                "id": "todo-6",
                "title": "Mezuniyet fotoğrafı yükle",
                "category": "ozluk",
                "due_date": "2026-06-01",
                "is_done": False,
                "priority": "low",
...
_APPLICATIONS: list[dict] = [
    {
        "id": "app-001",
        "application_type": "mazeret_sinav",
        "status": "onaylandi",
        "submitted_at": "2026-03-10T14:00:00",
        "course_code": "YBS201",
        "notes": "Sağlık raporu ektedir.",
        "reviewed_by": "Dr. Ayşe Yılmaz",
        "reviewed_at": "2026-03-12T10:00:00",
    },
]
...
@student_router.post(
    "/me/applications", status_code=status.HTTP_201_CREATED, summary="Başvuru gönder"
)
        ...
        "schedule": [
            {
                ...
                "end": "10:50",
                "course_code": "ENG101",
                "course_name": "İngilizce Hazırlık I",
                "classroom": "D-101",
            ...
            },
            {
                ...
                "end": "14:50",
                "course_code": "ENG103",
                "course_name": "Dinleme ve Konuşma I",
                "classroom": "D-201",
            ...
            },
            {
                ...
                "end": "12:50",
                "course_code": "ENG102",
                "course_name": "Okuma ve Yazma I",
                "classroom": "D-302",
                "instructor": "Öğr. Gör. Emily Brown",
            },
            {
                "day": "Perşembe",
                "start": "09:00",
...
@student_router.post(
    "/me/internship", status_code=status.HTTP_201_CREATED, summary="Staj başvurusu"
)
async def student_create_internship(
    body: InternshipCreate, user=Depends(get_verified_user)
):
    row = {
...
@student_router.post(
    "/me/credit-transfer",
    status_code=status.HTTP_201_CREATED,
    summary="İntibak başvurusu gönder",
)
async def student_create_credit_transfer(
    body: CreditTransferCreate, user=Depends(get_verified_user)
):
    row = {
...
@academic_user_router.get(
    "/me/sections/{section_id}/students", summary="Şube öğrenci listesi"
)
...
@academic_user_router.post(
    "/sections/{section_id}/exams",
    status_code=status.HTTP_201_CREATED,
    summary="Sınav tanımla",
)
async def academic_create_exam(
    section_id: str, body: ExamCreate, _user=Depends(get_verified_user)
):
    row = {
...
@academic_user_router.post(
    "/sections/{section_id}/grades/finalize", summary="Notları kesinleştir"
)
...
@academic_user_router.patch(
    "/approval-requests/{request_id}", summary="Onayla / Reddet"
)
async def academic_approve_request(
    request_id: str, body: ApprovalAction, user=Depends(get_verified_user)
):
...
async def academic_put_grades(
    section_id: str, body: GradeInput, user=Depends(get_verified_user)
):
    section = next((s for s in _SECTIONS if s["id"] == section_id), None)
    if not section:
        raise HTTPException(status_code=404, detail="Şube bulunamadı")
...
async def academic_put_attendance(
    section_id: str, body: AttendanceInput, user=Depends(get_verified_user)
):
    section = next((s for s in _SECTIONS if s["id"] == section_id), None)
    if not section:
        raise HTTPException(status_code=404, detail="Şube bulunamadı")
...
@academic_user_router.post(
    "/announcements", status_code=status.HTTP_201_CREATED, summary="Duyuru oluştur"
)
async def academic_create_announcement(
    body: AcademicAnnouncementCreate, user=Depends(get_verified_user)
):
    row = {
...
async def admin_create_department(
    body: DepartmentCreate, _user=Depends(get_admin_user)
):
...
async def admin_create_section(
    body: CourseSectionCreate, _user=Depends(get_admin_user)
):
    row = {
...
async def admin_create_calendar_event(
    body: CalendarEventCreate, _user=Depends(get_admin_user)
):
...
async def admin_update_registration_settings(
    body: RegistrationSettingsUpdate, _user=Depends(get_admin_user)
):
...
async def admin_create_announcement(
    body: AdminAnnouncementCreate, _user=Depends(get_admin_user)
):
    row = {
        "id": f"ann-{uuid.uuid4().hex[:8]}",
        "title": body.title,
...
async def admin_complete_document_request(
    request_id: str, _user=Depends(get_admin_user)
):
...
_MOCK_USERS: list[dict] = [
    ...
    {
        "id": "usr-4",
        "email": "mehmet.demir@dou.edu.tr",
        "full_name": "Prof. Mehmet Demir",
        "role": "Akademisyen",
        "is_active": True,
    ...
    {
        "id": "usr-6",
        "email": "zeynep.arslan@dou.edu.tr",
...
_MOCK_ROLES: list[dict] = [
    ...
    },
    {
        "id": "role-2",
        "name": "Akademisyen",
        "description": "Not, yoklama, danışmanlık",
        "permissions": [
            "grades.get",
            "grades.put",
            "attendance.get",
    ...
    },
    {
        "id": "role-3",
        "name": "Öğrenci",
        "description": "Kendi verilerini görme ve ders kayıt",
        "permissions": [
            "courses.get",
            "grades.get",
            "attendance.get",
...
async def admin_list_users(
    ...
    if search:
        s = search.lower()
        users = [
            u for u in users if s in u["full_name"].lower() or s in u["email"].lower()
        ]
...
async def admin_update_user(
    user_id: str, body: UserUpdate, _user=Depends(get_admin_user)
):
...
async def admin_update_role_permissions(
    role_id: str, body: RolePermissionsUpdate, _user=Depends(get_admin_user)
):
