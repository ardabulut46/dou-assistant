<script lang="ts">
	...
	let courseNoteDocs: Array<{ id: string; filename: string; meta?: Record<string, unknown>; size?: number }> =
		[];
	/** Ders notları: öğrencinin tüm kayıtları (şube eşlemesi için). */
	let noteEnrollments: DouEnrollment[] = [];
	/** Ders notları: seçili dönem (varsayılan aktif dönem). */
	...
	async function reloadCourseNotes() {
		...
		const kbs = (res?.items ?? res?.knowledges ?? res?.knowledge_bases ?? []) as Array<{
			id: string;
			name?: string;
		}>;
		const kbId = kbs.find((k) => (k?.name ?? '').trim() === COURSE_NOTES_KB_NAME)?.id ?? null;
		if (!kbId) {
			courseNoteDocs = [];
			return;
		}
		const filesRes = await searchKnowledgeFilesById(
			token,
			kbId,
			null,
			null,
		...
		courseNoteDocs = items.map((f) => ({ id: f.id, filename: f.filename, meta: f.meta, size: f.size }));
	}

	/** Seçili dönemde öğrencinin kayıtlı olduğu şubeler (section_id → kayıt). */
	$: enrolledNoteSectionMap = new Map(
		noteEnrollments
			.filter((e) => !selectedNotesTermId || e.term_id === selectedNotesTermId)
			.map((e) => [String(e.section_id), e] as const)
	);

	/**
	 * Ders notları, yalnızca öğrencinin kayıtlı olduğu şubeler için ders/şube bazında gruplanır.
	 * Böylece öğrenci sadece kendi hocasının/şubesinin notlarını görür.
	 */
	$: courseNoteGroups = (() => {
		const groups = new Map<
			string,
			{ enr: DouEnrollment; docs: typeof courseNoteDocs }
		>();
		for (const f of courseNoteDocs) {
			const sid = fileSectionId(f);
			const enr = enrolledNoteSectionMap.get(sid);
			if (!enr) continue;
			if (!groups.has(sid)) groups.set(sid, { enr, docs: [] });
			groups.get(sid)!.docs.push(f);
		}
	...
	async function loadPage() {
		...
		try {
			...
			if (apiKey === 'course-notes') {
				...
				terms = tl;
				if (!selectedNotesTermId) {
					selectedNotesTermId =
						tl.find((t) => t.is_active)?.id ?? tl[tl.length - 1]?.id ?? '';
				}
				const enr = await getDouStudentEnrollments(token).catch(() => null);
				noteEnrollments = enr?.enrollments ?? [];
				await reloadCourseNotes();
			}
			...
			<div class="space-y-4">
				...
				>
					<div class="mb-3 flex items-center gap-3">
						...
						<div class="min-w-0 flex-1">
							<div class="text-sm font-bold text-slate-800 dark:text-slate-100">Ders Notları</div>
							<div class="text-xs text-slate-500 dark:text-slate-400">
								Aldığın derslerin hocalarının paylaştığı PDF'leri görüntüleyip indirebilirsin.
							</div>
						</div>
						{#if terms.length}
							<select
								class="shrink-0 rounded-xl border border-black/10 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm focus:border-indigo-400 focus:outline-none dark:border-white/10 dark:bg-slate-900/40 dark:text-slate-200"
					...
					{#if courseNoteGroups.length}
						<div class="space-y-4">
							{#each courseNoteGroups as g}
								<div class="rounded-xl border border-black/10 dark:border-white/10">
									<div
										class="flex items-center gap-2 border-b border-black/5 bg-slate-50 px-3 py-2.5 dark:border-white/10 dark:bg-white/5"
									>
										<span class="font-mono text-xs font-bold text-indigo-600 dark:text-indigo-300">
											{g.enr.course_code}
										</span>
										...
										{#if g.enr.section_no}
											<span class="shrink-0 text-[11px] text-slate-400">· Şube {g.enr.section_no}</span>
										{/if}
										{#if g.enr.instructor_name}
											<span class="ml-auto shrink-0 truncate text-[11px] text-slate-400">
												{g.enr.instructor_name}
											</span>
										{/if}
									</div>
									<div class="space-y-2 p-3">
										{#each g.docs as f}
											...
											>
												<div
													class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-300"
												>
					...
					{:else}
						<div
							class="rounded-xl border border-dashed border-black/15 bg-slate-50 px-4 py-10 text-center dark:border-white/15 dark:bg-slate-900/30"
						>
							<div class="text-sm font-medium text-slate-600 dark:text-slate-300">
								Bu döneme ait ders notu yok
							</div>
							<div class="mt-1 text-xs text-slate-400">
								Hocaların aldığın derslere PDF eklediğinde burada görünecek.
							</div>
						</div>
					{/if}
				</div>
